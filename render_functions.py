...
def render_bar(console: Console, current_value: int, maximum_value: int, total_width: int, y: int) -> None:
    bar_width = int(float(current_value) / maximum_value * total_width)

    console.draw_rect(x=0, y=y, width=total_width, height=1, ch=1, bg=colour.bar_empty)

    if bar_width > 0:
        console.draw_rect(x=0, y=y, width=bar_width, height=1, ch=1, bg=colour.bar_filled)

    console.print(x=1, y=y, string=f"HP: {current_value}/{maximum_value}", fg=colour.bar_text)
